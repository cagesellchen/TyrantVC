...
class DockableWidget(MayaQWidgetDockableMixin, QWidget):
  def __init__(self, parent=None):
    ...
    self.setWindowTitle('Custom Maya Mixin Workspace Control 2')
  ...
  def deleteInstances(self):
    print "hello"
    mainWindow = wrapInstance(long(omui.MQtUtil.mainWindow()), QMainWindow)
    
    for obj in mainWindow.children():
        print "hello2"
        try:
            ...
            print objName  
            if objName == 'TyrantVC7':
                mainWindow.removeDockWidget(obj)
                obj.setParent(None)
                obj.deleteLater()
...
def DockableWidgetUIScript(restore=False):
  ...
  if customMixinWindow is None:
      ...
      customMixinWindow.setObjectName('TyrantVC8')
